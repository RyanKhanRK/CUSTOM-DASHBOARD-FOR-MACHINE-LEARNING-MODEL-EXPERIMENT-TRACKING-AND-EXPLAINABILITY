...
def run_multiple_models():
    ...
    # Count outliers across all numeric columns using the 1.5×IQR fence.
    # If the total number of outlier cells exceeds 100, remove every row
    # that contains at least one outlier value in any numeric column.
    ...
    if total_outlier_cells > 100:
        ...
        print(f"Outlier removal triggered ({total_outlier_cells} outlier cells > 100 threshold).")
        print(f"Removed {rows_removed} rows containing outliers. New shape: {df.shape}")
    else:
        print(f"Outlier count ({total_outlier_cells}) is within threshold (≤100). No rows removed.")
    # ── END OUTLIER REMOVAL ───────────────────────────────────────────────────

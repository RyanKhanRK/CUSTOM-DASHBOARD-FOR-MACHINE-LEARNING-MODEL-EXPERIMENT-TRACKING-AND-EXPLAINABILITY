...
async function handleFileSelected(file) {
    ...
    // Compute dataset stats (null count + outlier count)
    try {
        await computeDatasetStats(file);
    } catch (error) {
...
async function computeDatasetStats(file) {
    console.log("computeDatasetStats called")
    return new Promise((resolve) => {
        Papa.parse(file, {
            ...
            complete: function(results) {
                ...
                // Count outliers using IQR per numeric column
                let outlierCount = 0;
                fields.forEach(field => {
                    const values = rows
                        .map(row => row[field])
                ...
                multiModelState.datasetStats = { nullCount, outlierCount };
                console.log(nullCount, outlierCount)
                renderDatasetStats();
                resolve();
            },
            error: resolve  // Don't block on error
        });
    });
}

function renderDatasetStats() {
    const container = document.getElementById('dataset-stats-badges');
    if (!container) return;

    const { nullCount = 0, outlierCount = 0 } = multiModelState.datasetStats || {};

    const nullColor = nullCount === 0 ? '#22c55e' : '#f59e0b';
    const outlierColor = outlierCount === 0 ? '#22c55e' : '#ef4444';

    container.innerHTML = `
        <span style="
            display: inline-flex; align-items: center; gap: 6px;
            padding: 5px 14px;
            background: ${nullCount === 0 ? '#f0fdf4' : '#fffbeb'};
            border: 1px solid ${nullColor}40;
            border-radius: 20px;
            font-size: 12px;
            font-weight: 600;
            color: ${nullColor};
        ">
            <span style="font-size: 14px;">${nullCount === 0 ? '✅' : '⚠️'}</span>
            ${nullCount.toLocaleString()} Null Value${nullCount !== 1 ? 's' : ''}
        </span>
        <span style="
            display: inline-flex; align-items: center; gap: 6px;
            padding: 5px 14px;
            background: ${outlierCount === 0 ? '#f0fdf4' : '#fef2f2'};
            border: 1px solid ${outlierColor}40;
            border-radius: 20px;
            font-size: 12px;
            font-weight: 600;
            color: ${outlierColor};
        ">
            <span style="font-size: 14px;">${outlierCount === 0 ? '✅' : '🔴'}</span>
            ${outlierCount.toLocaleString()} Outlier${outlierCount !== 1 ? 's' : ''}
        </span>
    `;
...
function clearAllSelections() {
    ...
    multiModelState.datasetStats = null;
    const statsContainer = document.getElementById('dataset-stats-badges');
    if (statsContainer) {
